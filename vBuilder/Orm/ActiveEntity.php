...
class ActiveEntity extends Entity implements Nette\Security\IResource {
	...
	const STATE_NOT_LOADED = 4; // Document does not exist (failed to load)
	...
	public function load() {
		...
		if(!$this->repository->load($this)) {
			$this->state = self::STATE_NOT_LOADED;
	...
	public function exists() {
		...
		if($this->state == self::STATE_NOT_LOADED) return false;
	...
	public function getResourceId() {
		if($this->checkIfIdIsDefined() && count($this->metadata->getIdFields()) > 0) {
			...
			if(($acl instanceof Nette\Security\Permission || $acl instanceof vBuilder\Security\Authorizators\AclAuthorizator) && !$acl->hasResource($resId)) {
